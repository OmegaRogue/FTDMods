...
// 		#endregion
		...
		public string VariablesForBlocks() => string.Join('\n', from assembly in AppDomain.CurrentDomain.GetAssemblies()
														   from type in assembly.GetTypes()
														   where type.IsSubclassOf(typeof(Block))
														   from pair in GetterSourceFinder.GetVariablesForBlockType(type)
														   select
															   $"block: {type.FullName} attributename: {pair.variable.Attribute.Name} propertyname: {pair.variable.Property.Name} variable: {pair.variable} set: {pair.set}");
		
		public string ReadablesForBlocks() => string.Join('\n', from assembly in AppDomain.CurrentDomain.GetAssemblies()
																from type in assembly.GetTypes()
																where type.IsSubclassOf(typeof(Block))
																from pair in GetterSourceFinder.GetReadablesForBlockType(type)
																select
																	$"block: {type.FullName} {pair.property.Name} {pair.attribute.Name}");
